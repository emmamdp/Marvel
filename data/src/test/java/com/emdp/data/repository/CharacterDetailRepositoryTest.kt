...
class CharacterDetailRepositoryTest : KoinTest {
    ...
    fun `check that if data-source response connection fails, an error is returned`() =
        runBlocking {
            // Given
            whenever(mockCharactersDataSource.getCharacterDetailResponse(1011334))
                .doReturn(FailureBo.NoConnection.left())
            // When
            val actualResutl = repository.getCharacterDetail(1011334)
            // Then
    ...
    fun `check tha if data-source response is successful, a List of CharactersBo is returned`() =
        runBlocking {
            ...
            // When
            val actualResutl = mockCharactersDataSource.getCharacterDetailResponse(1011334)
            // Then
